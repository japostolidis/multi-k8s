...
docker build -t japostolidis/multi-server-k8s-pgfix:latest -t japostolidis/multi-server-k8s-pgfix:$SHA -f ./server/Dockerfile ./server
...
docker push japostolidis/multi-server-k8s-pgfix:latest
docker push japostolidis/multi-worker-k8s:latest

docker push japostolidis/multi-client-k8s:$SHA
docker push japostolidis/multi-server-k8s-pgfix:$SHA
docker push japostolidis/multi-worker-k8s:$SHA

kubectl apply -f k8s
kubectl set image deployments/server-deployment server=japostolidis/multi-server-k8s-pgfix:$SHA
